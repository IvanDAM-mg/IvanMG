<!-- BANNER -->
<div align="center">
  <img src="assets/ivan_banner_epic.gif" width="100%" alt="Ivan"/>
</div>

<br/>

<!-- LANGUAGE SELECTOR -->
<div align="center">

[![English](https://img.shields.io/badge/%F0%9F%87%AC%F0%9F%87%A7%20English-0A66C2?style=for-the-badge&labelColor=0A66C2)](https://github.com/IvanDAM-mg#-english)&nbsp;&nbsp;
[![Español](https://img.shields.io/badge/%F0%9F%87%AA%F0%9F%87%B8%20Espa%C3%B1ol-C0392B?style=for-the-badge&labelColor=C0392B)](https://github.com/IvanDAM-mg#-espa%C3%B1ol)

</div>

<!-- ABOUT ME -->
###👨‍💻 About me

I'm **Ivan**, finishing my Advanced Vocational Training in **Multiplatform Application Development (DAM)**.

During my studies I completed an internship as a programmer at **[Ordinaly Software](https://ordinaly.ai/en)**, a Seville-based startup focused on automating and optimizing business processes through Artificial Intelligence — my first real experience in a professional tech environment.

Throughout my training I have worked on both academic and industry projects. The one I'm most proud of is my **Final Year Project, ZIO Autos** — a mobile app currently in active development.

I've also built other projects along the way, like an **Android nutrition and diet app**, giving me solid hands-on experience before jumping into Flutter.

Outside of programming, I'm passionate about nature and cars — two things that, unsurprisingly, ended up inspiring my biggest project.

<br/>

---

###🗂️ Featured Project
**ZIO Autos** is my Final Year Project and my most personal bet as a developer.

I created it because I'm genuinely passionate about the automotive world — and I couldn't find an app that had everything I wanted. So I built it myself.

What makes it ambitious isn't just the idea — it's the execution. I chose **Flutter / Dart**, a language I had never used before, to ship a **single codebase for both Android and iOS**. I taught myself Flutter while building the actual product. No tutorial project — straight into the real thing.

ZIO Autos includes features that very few apps in this space offer, or that simply don't exist elsewhere. The end goal: launch on **Play Store** and **App Store**.

<div align="center">

| | |
|:--|:--|
|🧩 Language | Flutter · Dart |
|📲 Platform | Android · iOS — single codebase |
|⚙️ Status | Active development |
|🎯 Goal | Play Store + App Store release |

</div>

<br/>

---

### Tech Skills

<div align="center">

<table>
<tr>
<td align="center" width="120px"><b>Mobile</b></td>
<td>
<img src="https://skillicons.dev/icons?i=flutter,dart,androidstudio&perline=10" height="48"/>
</td>
</tr>
<tr>
<td align="center"><b>Frontend</b></td>
<td>
<img src="https://skillicons.dev/icons?i=react,nextjs,angular,ionic,html,css,js,ts,bootstrap&perline=10" height="48"/>
</td>
</tr>
<tr>
<td align="center"><b>Backend</b></td>
<td>
<img src="https://skillicons.dev/icons?i=spring,java,python&perline=10" height="48"/>
</td>
</tr>
<tr>
<td align="center"><b>Database</b></td>
<td>
<img src="https://skillicons.dev/icons?i=postgres,mysql&perline=10" height="48"/>
&nbsp;
<img src="https://img.shields.io/badge/MySQL_Workbench-4479A1?style=flat-square&logo=mysql&logoColor=white&labelColor=4479A1" height="28"/>
&nbsp;
<img src="https://img.shields.io/badge/Odoo-714B67?style=flat-square&logo=odoo&logoColor=white" height="28"/>
</td>
</tr>
<tr>
<td align="center"><b>Tools</b></td>
<td>
<img src="https://skillicons.dev/icons?i=git,github,vscode,idea,postman,eclipse&perline=10" height="48"/>
&nbsp;
<img src="https://img.shields.io/badge/NetBeans-1B6AC6?style=flat-square&logo=apache-netbeans-ide&logoColor=white" height="28"/>
&nbsp;
<img src="https://img.shields.io/badge/Terminal-000000?style=flat-square&logo=gnubash&logoColor=white" height="28"/>
</td>
</tr>
<tr>
<td align="center"><b>Automation</b></td>
<td>
<img src="https://img.shields.io/badge/N8N-EA4B71?style=flat-square&logo=n8n&logoColor=white" height="28"/>
</td>
</tr>
</table>

</div>

<br/>

---

### Contact

<div align="center">

Got a project, an idea, or just want to reach out?

<br/><br/>

<a href="https://www.linkedin.com/in/ivan-moreno-giles-499a49386/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" height="40"/>
</a>
&nbsp;&nbsp;&nbsp;
<a href="mailto:imorenogiles3@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" height="40"/>
</a>

</div>

<br/>

---

### 👨‍💻 Sobre mí

Soy **Iván**, terminando el **Grado Superior de Desarrollo de Aplicaciones Multiplataforma (DAM)**.

Durante mis estudios realicé mis prácticas como programador en **[Ordinaly Software](https://ordinaly.ai)**, una startup sevillana centrada en la automatización y optimización de procesos empresariales mediante Inteligencia Artificial — mi primer contacto real con el mundo profesional del desarrollo.

A lo largo de la formación he adquirido experiencia práctica tanto en proyectos académicos como en la industria. El que más me enorgullece es mi **Proyecto de Fin de Ciclo, ZIO Autos** — una app móvil en desarrollo activo.

También he creado otros proyectos, como una **app de Android de dietas y nutrición**, que me dio experiencia práctica en desarrollo móvil antes de meterme de lleno en Flutter.

Fuera de la programación, me apasionan la naturaleza y los coches — dos cosas que, no por casualidad, acabaron inspirando mi proyecto más grande.

<br/>

---

### 🗂️ Proyecto a destacar

**ZIO Autos** es mi Proyecto de Fin de Ciclo y mi apuesta más personal como desarrollador.

Lo creé porque soy un apasionado del mundo del motor — y no encontraba una app que tuviese todo lo que buscaba. Así que la construí yo mismo.

Lo ambicioso no es solo la idea — es la ejecución. Decidí desarrollarla en **Flutter / Dart**, un lenguaje que no conocía de antes, para lanzar una **única base de código tanto para Android como para iOS**. Aprendí Flutter mientras construía el producto real. Sin proyecto tutorial aparte — directamente al grano.

ZIO Autos incluye funcionalidades que muy pocas apps del sector tienen, o que directamente no existen. El objetivo final: lanzarla en **Play Store** y **App Store**.

<div align="center">

| | |
|:--|:--|
|🧩 Lenguaje | Flutter · Dart |
|📲 Plataforma | Android · iOS — un solo código |
|⚙️ Estado | En desarrollo activo |
|🎯 Objetivo | Lanzamiento en Play Store + App Store |

</div>

<br/>

---

### Habilidades técnicas

<div align="center">

<table>
<tr>
<td align="center" width="120px"><b>Móvil</b></td>
<td>
<img src="https://skillicons.dev/icons?i=flutter,dart,androidstudio&perline=10" height="48"/>
</td>
</tr>
<tr>
<td align="center"><b>Frontend</b></td>
<td>
<img src="https://skillicons.dev/icons?i=react,nextjs,angular,ionic,html,css,js,ts,bootstrap&perline=10" height="48"/>
</td>
</tr>
<tr>
<td align="center"><b>Backend</b></td>
<td>
<img src="https://skillicons.dev/icons?i=spring,java,python&perline=10" height="48"/>
</td>
</tr>
<tr>
<td align="center"><b>Base de datos</b></td>
<td>
<img src="https://skillicons.dev/icons?i=postgres,mysql&perline=10" height="48"/>
&nbsp;
<img src="https://img.shields.io/badge/MySQL_Workbench-4479A1?style=flat-square&logo=mysql&logoColor=white" height="28"/>
&nbsp;
<img src="https://img.shields.io/badge/Odoo-714B67?style=flat-square&logo=odoo&logoColor=white" height="28"/>
</td>
</tr>
<tr>
<td align="center"><b>Herramientas</b></td>
<td>
<img src="https://skillicons.dev/icons?i=git,github,vscode,idea,postman,eclipse&perline=10" height="48"/>
&nbsp;
<img src="https://img.shields.io/badge/NetBeans-1B6AC6?style=flat-square&logo=apache-netbeans-ide&logoColor=white" height="28"/>
&nbsp;
<img src="https://img.shields.io/badge/Terminal-000000?style=flat-square&logo=gnubash&logoColor=white" height="28"/>
</td>
</tr>
<tr>
<td align="center"><b>Automatización</b></td>
<td>
<img src="https://img.shields.io/badge/N8N-EA4B71?style=flat-square&logo=n8n&logoColor=white" height="28"/>
</td>
</tr>
</table>

</div>

<br/>

---

### Contacto

<div align="center">

¿Tienes un proyecto, una idea o simplemente quieres contactar?

<br/><br/>

<a href="https://www.linkedin.com/in/ivan-moreno-giles-499a49386/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" height="40"/>
</a>
&nbsp;&nbsp;&nbsp;
<a href="mailto:imorenogiles3@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" height="40"/>
</a>

<div align="center">
<sub>always building something · siempre construyendo algo</sub>
</div>
